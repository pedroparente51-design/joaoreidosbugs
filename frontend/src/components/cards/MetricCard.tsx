      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-6 sm:gap-4 mb-8 md:mb-10">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-1">
            <h3 className="text-[14px] md:text-[15px] font-black text-white tracking-tight leading-tight">Base de pessoas cadastradas em nosso dash</h3>
            <span className="text-[10px] md:text-[11px] text-slate-500 font-bold uppercase tracking-[0.1em]">Total de registros</span>
          </div>
        </div>
        <button className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/5 py-2 md:py-2.5 px-4 md:px-5 rounded-xl transition-all group/btn">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex justify-between items-end gap-2">
          <div className="flex flex-col gap-2">
             <span className="text-[9px] md:text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Cadastrados</span>
             <p className={cn(
               "text-3xl md:text-5xl font-black text-white tracking-tighter leading-none transition-all duration-500",
               isPulsing && "text-primary scale-105"
             )}>
               {isValuesVisible ? userCount.toLocaleString('pt-BR') : formatValue(userCount)}
             </p>
          </div>
          <div className="flex flex-col items-end gap-2">
             <span className="text-[9px] md:text-[10px] text-slate-500 font-black uppercase tracking-[0.2em]">Base Ativa</span>
             <p className="text-2xl md:text-[42px] font-black text-accent-blue tracking-tighter leading-none">
               {formatValue('100%')}
             </p>
          </div>
        </div>
        ...
        <div className="h-2.5 md:h-3 w-full bg-slate-900 rounded-full p-0.5 border border-white/5 overflow-hidden">
          <div 
            className="h-full rounded-full bg-gradient-to-r from-accent-blue to-primary shadow-[0_0_20px_rgba(14,165,233,0.3)] transition-all duration-1000"
            style={{ width: '100%' }}
          />
        </div>
      </div>
