...
import { Lock, Mail, User, ArrowRight, TrendingUp, AlertCircle } from "lucide-react";
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se o signup for bem sucedido e tiver auto-login
      if (data.session) {
        setStep("logging-in");
        await refreshUser();
        ...
        // Dependendo da config do Supabase, pode pedir confirmação de email
        setError("Conta criada! Confirme seu email para fazer o login.");
        setStep("idle");
        setLoading(false);
      }
        ...
        <div className="gateway-card rounded-[32px] p-10 border-white/10">
          ...
          <div className="mt-8 text-center">
            <p className="text-sm text-gray-500 font-medium">
              Já tem uma conta?{" "}
              <Link to="/login" className="text-white hover:text-primary font-bold transition-colors underline-offset-4 hover:underline">
                Fazer Login
              </Link>
            </p>
          </div>
